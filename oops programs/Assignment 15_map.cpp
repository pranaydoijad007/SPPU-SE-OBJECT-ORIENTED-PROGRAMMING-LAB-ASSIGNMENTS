// Write a C++ Program for Searching & Sorting Records Like Person(name, telephone,bdate)  Using STL
...
public:
	void getdata();
	void display();
	void Sort_Map(map<string,stud> &);	//Function to Sort Map<> data
};
void stud::getdata()
{
	cout<<"\nEnter Telephone No: ";
...
void stud::Sort_Map(map<string,stud> &m)
{
	...
	map<string,stud> m1;
	int cnt,i=0,j;
	//string name,tempbdate,tempdate1;
	...
	for(itr=m.begin();itr!=m.end();itr++)		//Display Map<> Data
		{
					s2=(*itr).second;
					cout<<(*itr).first<<"\t";
					s2.display();
	...
	string nm[10];
	itr1=m.begin();
	for(i=0;i<cnt;i++)							//Store <Key> in nm[] array
	{
		nm[i]=itr1->first;
		itr1++;
	}
	...
	for(i=0,itr=itr1=m.begin(),i<cnt;itr1!=m.end();itr1++,i++)
	{
		...
		if(((itr1->second).telno) < ((itr2->second).telno))			//Sort on Telephone No.
		{

			tempname = nm[i];										//Swap Name
			nm[i]=nm[j];
			nm[j]=tempname;


			temptelno=(itr1->second).telno;							//Swap Telephone No.
			(itr1->second).telno= (itr2->second).telno;
			(itr2->second).telno= temptelno;

			tempbdate = (itr1->second).bdate;						//Swap Birthday
